...
(function () {
  ...
  function openSocket() {
    ...
    sock.onmessage = function (ev) {
      ...
      if (msg.t === "state") {
        ...
        if (before && roundChanged(before, msg)) draft = "";
        render();
      } else if (msg.t === "error") {
  ...
  function roundChanged(a, b) {
    var ta = myTeam(a), tb = myTeam(b);
    if (!ta || !tb) return false;
    return ta.round !== tb.round || (ta.revealed && !tb.revealed);
  }
  ...
  function leave() {
    if (ws) { try { ws.onclose = null; ws.close(); } catch (e) {} }
    ws = null; view = null; code = null; status = "idle"; draft = "";
    clearTimeout(retryTimer);
    location.hash = "";
    render();
  }
  ...
  function masthead(onBrand) {
    return el("div", { class: "top" }, [
      el("button", { class: "brand", type: "button", onclick: onBrand || function () {} }, [
  ...
  function homeScreen() {
    ...
    return el("div", { class: "wrap" }, [
      masthead(),
      el("div", { class: "hero" }, [
        el("h1", { text: "Play something together" }),
        el("p", { class: "lede", text: "Family games for whoever is in the room, on whatever phone they're holding." })
      ]),

      el("div", { class: "game-list" }, [
        el("div", { class: "game-tile" }, [
          el("div", { class: "glyph", text: "🎯" }),
          el("div", { class: "meta" }, [
            el("b", { text: "Converge" }),
            el("span", { class: "tiny", text: "Everyone writes a word. Reveal together. Keep going until you all land on the same one." })
          ])
        ])
      ]),
      ...
      el("div", { class: "card" }, [
        ...
        el("hr", { class: "divider" }),
        el("div", { class: "field" }, [
          el("label", { class: "label", text: "Or join one" }),
          codeInput
        ]),
      ...
      el("p", { class: "tiny", text: "Everyone opens the same code on their own phone." })
    ]);
  }
  ...
  function writeCard(team) {
    ...
    var input = el("input", {
      ...
      placeholder: "your word", autocomplete: "off", spellcheck: "false",
    ...
    var body = mySubmission
      ...
      : [
          ...
          el("button", { class: "btn btn-primary btn-lg btn-block", type: "button", onclick: submit },
            [document.createTextNode("Lock it in")])
        ];
    ...
    return el("div", { class: "card" }, [
      ...
      el("p", { class: "tiny", text: team.round === 1
        ? "Everyone writes any word at all. You're trying to end up on the same one."
        : "Write the word that bridges the last two. No repeats." }),
  ...
  function revealCard(team) {
    return el("div", { class: "card" }, [
      ...
      el("p", { class: "verdict no", text: "Not the same word — go again." }),
      el("div", { class: "btn-row" }, [
  ...
  function historyCard(team) {
    return el("div", { class: "card tight" }, [
      el("span", { class: "label", text: "How we got here" }),
  ...
  if (/^[A-Z]{4}$/.test(initial)) {
    ...
    else { joinCode = initial; render(); toast("Add your name, then join."); }
  }
})();
